...
export async function criarPacote(req: Request, res: Response) {
    ...
    if (!tipo || valor_cobrado === undefined) {
        return res.status(400).json({ erro: 'Tipo do pacote e valor cobrado são obrigatórios.' })
    }

    if (!['INDIVIDUAL', 'PACOTE_5', 'PACOTE_10'].includes(tipo)) {
        return res.status(400).json({ erro: 'Tipo de pacote inválido. Opções: INDIVIDUAL, PACOTE COM 5, PACOTE COM 10.' })
    }
...
export async function adicionarSessao(req: Request, res: Response) {
    ...
    if (!data || !horario || !slot) {
        return res.status(400).json({ erro: 'Data, horário e slot são obrigatórios para adicionar uma sessão.' })
    }
...
export async function marcarPagamento(req: Request, res: Response) {
    ...
    if (!['AGUARDANDO', 'PAGO'].includes(status_pagamento)) {
        return res.status(400).json({ erro: 'Status de pagamento inválido. Opções: AGUARDANDO, PAGO.' })
    }
